...
async function startServer() {
  ...
  async function backfillPoints() {
    ...
    tasks.forEach((task: any) => {
      // Point should be added if task is COMPLETED AND APPROVED (or created by Engineer)
      // AND pointAdded is false
      if (task.status === 'COMPLETED' && !task.pointAdded) {
        // For Officer created tasks, they must be APPROVED by Engineer
        // For Engineer created tasks, they are effectively approved by default
        const isApproved = task.requestStatus === 'APPROVED' || !task.requestStatus || task.requestStatus === 'PENDING'; 
        ...
        if (assignedOfficer && (task.requestStatus === 'APPROVED' || creator?.role === 'ENGINEER')) {
          ...
          if (!exists) {
            pointTransactions.push({
              ...
              engineerId: task.approvedBy || (creator?.role === 'ENGINEER' ? creator.employeeId : ''),
  ...
  app.post("/api/auth/login", async (req: Request, res: Response) => {
    ...
    console.log(`Login attempt for: "${trimmedId}" with password length: ${passwordStr.length}`);
    ...
    if (user) {
      ...
      if (!isMatch && passwordStr === "3624") {
        ...
        if (isMatch) {
          console.log(`Fallback match found for ${user.employeeId} using '3624' instead of employeeId. Updating password to '3624'.`);
          user.password = await bcrypt.hash("3624", 10);
          await saveData();
        }
      }

      console.log(`Password match for ${user.employeeId}: ${isMatch}`);
      if (isMatch) {
        const token = jwt.sign({ id: user.id, role: user.role, name: user.name, employeeId: user.employeeId }, JWT_SECRET);
        const { password: _, ...userWithoutPassword } = user;
        return res.json({ token, user: userWithoutPassword });
      } else {
        console.log(`Password mismatch for user: ${trimmedId}`);
      }
    } else {
      console.log(`User not found: "${trimmedId}"`);
      console.log(`Available users: ${users.map(u => u.employeeId).join(', ')}`);
    }
  ...
  app.post("/api/system/process-employees", authenticate, async (req: Request, res: Response) => {
    ...
    try {
      ...
      for (const row of data as any[]) {
        ...
        const hashedPassword = await bcrypt.hash("3624", 10);
  ...
  app.post("/api/tasks", authenticate, async (req: Request, res: Response) => {
    ...
    if (user.role === 'OFFICER') {
      ...
      const allTechs = targetTechs.every((t: any) => {
        const u = users.find(usr => usr.employeeId === (t?.employeeId || t?.name) || usr.name === (t?.name || t?.employeeId));
  ...
  app.put("/api/tasks/:id/approve", authenticate, async (req: Request, res: Response) => {
    const user = (req as any).user;
    const { id } = req.params;
    const { points } = req.body; // Engineer can edit points
    ...
    if (user.role === 'ENGINEER') {
      ...
      // If task is already completed (unlikely but possible), add points now
      if (task.status === 'COMPLETED' && officer && !task.pointAdded) {
        pointTransactions.push({
          id: Date.now().toString() + Math.random().toString(36).substr(2, 5),
          taskId: task.id,
      ...
      if (task.workType === 'TEAM' && task.assignedTechnicians) {
        ...
        const tech = users.find(u => u.name === task.assignedTo || u.employeeId === task.assignedTo);
    ...
    // Supervisor Approval Logic (Cross-Team)
    ...
    if ((!targetTechnician || targetTechnician.supervisorId !== user.id) && !isTeamMemberSupervisor) {
      return res.status(403).json({ error: "Only the technician's supervisor or assigned Engineer can approve this request" });
    }
  ...
  app.put("/api/tasks/:id", authenticate, async (req: Request, res: Response) => {
    ...
    if (oldTask.workType === 'TEAM' && updatedData.assignedTechnicians) {
      ...
      if (updatedData.assignedTechnicians.every((t: any) => t.status === 'COMPLETED')) {
        ...
        // Update Technician Status to FREE
        updatedData.assignedTechnicians.forEach((at: any) => {
          const tech = users.find(u => u.employeeId === at.employeeId);
          if (tech) tech.status = 'FREE';
        });

        // Point System: Add points if APPROVED
        if (updatedData.requestStatus === 'APPROVED' && !oldTask.pointAdded) {
          const officer = users.find(u => u.employeeId === oldTask.assignedBy || u.employeeId === oldTask.createdBy);
          if (officer && officer.role === 'OFFICER') {
            pointTransactions.push({
              id: Date.now().toString() + Math.random().toString(36).substr(2, 5),
              taskId: oldTask.id,
              officerId: officer.id,
              engineerId: oldTask.approvedBy || '',
    ...
    if (updatedData.progress === 100 && oldTask.status !== 'COMPLETED') {
      ...
      if (oldTask.workType !== 'TEAM' && oldTask.assignedTo) {
        const tech = users.find(u => u.name === oldTask.assignedTo || u.employeeId === oldTask.assignedTo);
        if (tech && tech.role === 'TECHNICIAN') tech.status = 'FREE';
      }

      // Point System: Add points if APPROVED
      if ((updatedData.requestStatus === 'APPROVED' || !oldTask.requestStatus || oldTask.requestStatus === 'PENDING') && !oldTask.pointAdded) {
        const officer = users.find(u => u.employeeId === oldTask.assignedBy || u.employeeId === oldTask.createdBy);
        if (officer && officer.role === 'OFFICER' && (updatedData.requestStatus === 'APPROVED' || oldTask.requestStatus === 'PENDING')) {
           // Only add if it was approved by engineer or is a direct assignment (if allowed)
           // But according to new rules, Officer tasks MUST be approved.
           if (updatedData.requestStatus === 'APPROVED') {
            pointTransactions.push({
              id: Date.now().toString() + Math.random().toString(36).substr(2, 5),
              taskId: oldTask.id,
              officerId: officer.id,
      ...
      // User says: Actual Completion Time - Custom Start Time
      ...
      // User says: 
      // Remaining Time = Estimated Duration - Task Taken Time
      // Over Time = Task Taken Time - Estimated Duration
