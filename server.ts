...
async function startServer() {
  ...
  app.post("/api/tasks/:id/approve", authenticate, async (req: Request, res: Response) => {
    ...
    if (user.role === 'ENGINEER') {
      ...
      // Point System: Point added to Officer Profile only after Engineer APPROVE
      if (officer && !task.pointAdded) {
        pointTransactions.push({
          id: Date.now().toString() + Math.random().toString(36).substr(2, 5),
          taskId: task.id,
          ...
          taskPriority: task.urgency,
          completedAt: new Date().toISOString() // Using approval time as completion for point record if requested on approval
        });
        task.pointAdded = true;
      }
